(() => {
  ...
  exports.updateSensor = functions.database.ref('/sessions/{sessionId}/environment/data/{timestamp}').onWrite((event) => {
    const sessionId = event.params.sessionId;
    const sensorData = event.data.val();
    console.log(`Session[${sessionId}]: sensor data`, sensorData);
    const {temperature} = sensorData;
    const body = {
      temperature: {
        sensor_id: 'test-kevin',
        value: temperature,
        battery_level: 42,
        is_charging: 1,
      },
